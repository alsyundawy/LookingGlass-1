 ...
 */
$(document).ready(function(){$("#userip").click(function(){$("#host").val($("#userip").text())});$("#networktest").submit(function(){var b=$("input[name=host]").val(),var b2=$("input[name=port]").val(),d="cmd="+$("select[name=cmd]").val()+"&host="+b"&port="+b2;if(""==b)$("#hosterror").addClass("error");else{$("#submit").attr("disabled","true").text("Loading...");$("#response").html();var a=new XMLHttpRequest;a.open("GET","ajax.php?"+d,!0);a.send(null);var c;c=window.setInterval(function(){a.readyState==XMLHttpRequest.DONE&&(window.clearTimeout(c),
